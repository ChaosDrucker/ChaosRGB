...
/* ===================== WIFI CREDENTIALS ===================== */
const char* ssid = "deineSSID";           // Hier deinen WLAN-Namen eintragen (nur fuer Uhr-Funktion noetig)
const char* password = "deinPW";    // Hier dein WLAN-Passwort eintragen
...
bool stripGamesEnabled = false;  // External LED strip games (ENEMYS, THE LINE)
...
void resetToIdle() {
  ...
  if (stripGamesEnabled) {
    currentSystemGame = SYS_GAME_CHAOS;
    gameState = IDLE;
  } else {
    currentSystemGame = SYS_GAME_SIMON;
    gameState = INTRO;
  }
...
void setup() {
  ...
  // Check for strip games activation: BLUE + RED held at boot (not all 4)
  if (digitalRead(BTN_BLUE) == LOW && digitalRead(BTN_RED) == LOW &&
      digitalRead(BTN_GREEN) == HIGH && digitalRead(BTN_YELLOW) == HIGH) {
    stripGamesEnabled = true;

    // Visual feedback
    display.clearDisplay();
    display.setTextSize(2);
    display.setCursor(centerX("STRIP", 2), 8);
    display.print("STRIP");
    display.setCursor(centerX("GAMES", 2), 26);
    display.print("GAMES");
    display.setCursor(centerX("ON!", 2), 46);
    display.print("ON!");
    display.display();
...
void handleSystemGreenLongPress() {
  ...
  if (greenLongPressActive && !greenLongPressDone &&
    ...
    // Switch game - skip external strip games if not enabled at boot
    do {
      currentSystemGame = (SystemGame)((currentSystemGame + 1) % NUM_GAMES);
    } while (!stripGamesEnabled &&
             (currentSystemGame == SYS_GAME_ENEMYS || currentSystemGame == SYS_GAME_CHAOS));
...
void miniLineIdle() {
  ...
  // Highscore
  display.setTextSize(1);
  display.setCursor(centerX("HI-SCORE:",1), 38);
  display.print("HI-SCORE:");

  display.setTextSize(2);
  display.setCursor(centerX(String(miniLineHighScore),2), 46);
...
void raceGameLoop() {
  if (raceState == RACE_IDLE) {
    ...
    display.setTextSize(1);
    display.setCursor(centerX("THE", 1), 10);
    display.print("THE");
...
void countdownGameOver() {
  ...
  // Show difference
  display.setTextSize(1);
  display.setCursor(centerX("Difference:", 1), 38);
...
void speedGame() {
  ...
  if (speedTotalPresses > 0) {
    ...
    display.setTextSize(1);
    display.setCursor(centerX("TIME:", 1), 46);
    display.print("TIME:");
